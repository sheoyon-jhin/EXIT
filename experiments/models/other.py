...
class _GRU_forecasting(torch.nn.Module):
    def __init__(self, input_channels,output_time, hidden_channels, output_channels, use_intensity):
        ...
        # gru_channels = input_channels if use_intensity else (input_channels - 1) // 2
        gru_channels = input_channels + use_intensity  if use_intensity else (input_channels - 1) // 2
        self.gru_cell = torch.nn.GRUCell(input_size=gru_channels, hidden_size=hidden_channels)
    ...
    def forward(self, times, coeffs, final_index, z0=None):
        # import pdb ; pdb.set_trace()
        interp = controldiffeq.NaturalCubicSpline(times, coeffs)
        X = torch.stack([interp.evaluate(t) for t in times], dim=-2) # 200,24,7
        half_num_channels = (self.input_channels - 1) // 2
        ...
        out = self.linear(out[:,input_time-self.output_time:,:])
...
class ODERNN_forecasting(_GRU_forecasting):
    def __init__(self, input_channels,output_time, hidden_channels, output_channels, hidden_hidden_channels, num_hidden_layers,
                 use_intensity):
