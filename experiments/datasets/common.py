...
def preprocess_data_forecasting2(times, X, y, final_index, append_times):
    ...
    if append_times:
        augmented_X.append(times.unsqueeze(0).repeat(X.size(0), 1).unsqueeze(-1))
        times_y = torch.arange(times[-1],times[-1]+y.shape[1]).to(times)
        augmented_y.append(times_y.unsqueeze(0).repeat(y.size(0), 1).unsqueeze(-1))

    augmented_X.append(X)
    augmented_X[0] = augmented_X[0].cuda()
    augmented_y.append(y)
    augmented_y[0] = augmented_y[0].cuda()
    if len(augmented_X) == 1:
        y = augmented_y[0]
        X = augmented_X[0]
    else:
        X = torch.cat(augmented_X, dim=2)
        y = torch.cat(augmented_y, dim=2)
    if append_times:
...
def preprocess_data_forecasting3(times, X, y, final_index, append_times):
    ...
    print(f"test_y shape {test_y.shape}")
    # import pdb ; pdb.set_trace()
    # PATH = '/home/bigdyl/IDEA4/experiments/datasets/google_baseline/0/'
