...
def _evaluate_metrics_forecasting(name, dataloader, model, times,terminal_time, loss_fn, mode,epoch,time_l,num_classes, device, kwargs):
    with torch.no_grad():
        total_accuracy = 0
        # total_confusion = torch.zeros(num_classes, num_classes).numpy()  # occurs all too often
        total_dataset_size = 0
        ...
        for batch in dataloader:
            ...
            *coeffs, true_y, lengths = batch
            ...
            if name in BASELINE_MODELS:
                    pred_y, _ = model(times, coeffs, lengths, **kwargs)
            else:
                 pred_y, _ = model(times,terminal_time, coeffs, lengths, **kwargs)
                 if 'google' in name:
                     pred_y = pred_y[:,:,:-1]
...
def _train_loop(name,train_dataloader, val_dataloader,test_dataloader, model, times,terminal_time, optimizer,optimizer_t, loss_fn, max_epochs, num_classes,writer, device,
                kwargs, step_mode,learn_t,time_l='end'):
    ...
    for epoch in tqdm_range:
            ...
            with _SuppressAssertions(tqdm_range):
                ...
                *train_coeffs, train_y, lengths = batch
                ...
                if 'forecasting' in name:
                    ...
                    pred_y, reg = model(times,terminal_time, train_coeffs, lengths, **kwargs)
                    if 'google'in name:
                        # import pdb ; pdb.set_trace()
                        pred_y = pred_y[:,:,:-1]
                        train_y = train_y[:,:,:-1]
                ...
                writer.add_scalar('trains/kinetic_reg',reg, iterations)
                # writer.add_scalar('trains/start_time_tau', terminal_time[0].item(), iterations)
                
                writer.add_scalar('trains/terminal_time_tau', terminal_time[-1].item(), iterations)
                loss += reg
                loss.backward()
                if learn_t:
                    ...
                    # with torch.no_grad():
                    #     terminal_time[0] = fixed_start_time
        ...
        if epoch % epoch_per_metric == 0 or epoch == max_epochs - 1:
            model.eval()
            if 'forecasting' in name:
                train_metrics = _evaluate_metrics_forecasting(name,train_dataloader, model, times,terminal_time, loss_fn,'train',epoch,time_l, num_classes, device, kwargs)
                val_metrics = _evaluate_metrics_forecasting(name,val_dataloader, model, times,terminal_time, loss_fn,'val',epoch,time_l, num_classes, device, kwargs)
                test_metrics = _evaluate_metrics_forecasting(name,test_dataloader, model, times,terminal_time, loss_fn,'test',epoch,time_l, num_classes, device, kwargs)
                
                
                writer.add_scalar('validation/loss',val_metrics.loss,epoch)
               
                writer.add_scalar('validation/terminal_time_tau',terminal_time[-1].item(),epoch)
                
                writer.add_scalar('test/loss',test_metrics.loss,epoch)
                
                writer.add_scalar('test/terminal_time_tau',
                                terminal_time[-1].item(), epoch)
            else:
                train_metrics = _evaluate_metrics(name,train_dataloader, model, times,terminal_time, loss_fn,'train',epoch, num_classes, device, kwargs)
                val_metrics = _evaluate_metrics(name,val_dataloader, model, times,terminal_time, loss_fn,'val',epoch, num_classes, device, kwargs)
                test_metrics = _evaluate_metrics(name,test_dataloader, model, times,terminal_time, loss_fn,'test',epoch, num_classes, device, kwargs)
...
def main(name, times,weight_decay, train_dataloader, val_dataloader, test_dataloader, device, make_model, num_classes, max_epochs,
    ...
    terminal_time =torch.tensor(start_time,dtype=torch.float32,requires_grad=True,device=device)
    
    optimizer = torch.optim.Adam(model.parameters(), lr=lr,weight_decay=weight_decay)
    optimizer_T = torch.optim.SGD([terminal_time],lr=time_lr) # 1.0
    
    
    history,terminal_time = _train_loop(name,train_dataloader, val_dataloader,test_dataloader, model, times,terminal_time, optimizer,optimizer_T, loss_fn, max_epochs,
                          num_classes,writer, device, kwargs, step_mode,learn_t,time_l)
    ...
    if name == 'ncde':
        ...
        def make_model():
            ...
            model = models.NeuralCDE_IDEA4_forecasting(func=func_k,func_g = func_g,func_f = func_f, input_channels=input_channels, hidden_channels=hidden_channels,
                                     ...
                                     div_samples=div_samples, output_time = output_time,initial=initial)
            return model, func_k
    elif name =='idea4_gruode_forecasting':
        def make_model():
            func_k = models.GRU_ODE(input_channels=input_channels, hidden_channels=hidden_channels)
            func_g = models.FinalTanh_g2(input_channels=input_channels, hidden_channels=hidden_channels+input_channels,
                                            hidden_hidden_channels=hidden_hidden_channels,
                                            num_hidden_layers=num_hidden_layers)
            func_f = models.ODEFunc_f2(input_channels=input_channels, hidden_channels=hidden_channels+input_channels,
                                            hidden_hidden_channels=hidden_hidden_channels,
                                            num_hidden_layers=num_hidden_layers)
            model = models.NeuralCDE_IDEA4_forecasting(func=func_k,func_g = func_g,func_f = func_f, input_channels=input_channels, hidden_channels=hidden_channels,
                                     hidden_hidden_channels=hidden_hidden_channels,output_channels=output_channels,method=method,
                                     ...
                                     div_samples=div_samples, output_time = output_time,initial=initial)
            return model, func_k
    elif name =='idea4_gruode_forecasting2':
        def make_model():
            func_k = models.GRU_ODE(input_channels=input_channels, hidden_channels=hidden_channels)
            func_g = models.GRU_ODE_g(input_channels=input_channels, hidden_channels=hidden_channels)
            func_f = models.GRU_ODE_g(input_channels=input_channels, hidden_channels=hidden_channels)
            model = models.NeuralCDE_IDEA4_forecasting2(func=func_k,func_g = func_g,func_f = func_f, input_channels=input_channels, hidden_channels=hidden_channels,
                                     hidden_hidden_channels=hidden_hidden_channels,output_channels=output_channels,method=method,
                                     kinetic_energy_coef=kinetic_energy_coef, jacobian_norm2_coef=jacobian_norm2_coef, 
                                     div_samples=div_samples, output_time = output_time,initial=initial)
            return model, func_k
